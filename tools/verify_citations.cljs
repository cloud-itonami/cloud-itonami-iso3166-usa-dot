...
;; nest as substrings of one another (part `1` is a prefix of part `1201`, and
;; section `661.1` of `661.11`), so a string match can succeed against the
;; wrong node. This matters more than usual for DOT, whose part numbers collide
;; ACROSS titles too -- 49 CFR 661 and 23 CFR 661 are unrelated programmes. Walking explicit [type identifier] steps cannot pass by accident.
...
(def min-citations (js/parseInt (flag-val "--min" "55") 10))
(def min-quotes    (js/parseInt (flag-val "--min-quotes" "12") 10))
...
(defn quotes
  ...
  than one load-bearing claim (48 CFR 1201.104 excludes the FAA in (d) and
  releases the Maritime Administration in (c); 23 CFR 635.410 sets both a de
  minimis and a 25 percent alternate-bid path). Each span becomes its own
  check, so losing one of them is a failure rather than a silently smaller
  gate."
  []
  (for [[iso es] catalog, e es
...
(defn -main []
  (let [pos (positives) neg (negatives) qs (vec (quotes))]
    ...
    (when (empty? qs)
      (die 2 "CANNOT-ANSWER: no entry carries :statute/verified-quotes, so the"
